...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Defaults to prediction_models/results/test_results/reconstructed_scalers.npz if present."
        ),
    )
    parser.add_argument(
        "--scaler-data-dir",
...
def resolve_paths(args: argparse.Namespace) -> None:
    ...
    default_scaler = (
        args.repo / "prediction_models" / "results" / "test_results" / "reconstructed_scalers.npz"
    )
    ...
    if args.parameters is None:
        args.parameters = (
            args.repo / "source_data" / "R7_SIMLE" / "R7_ROBUSTNESS" / "parameters.json"
        )
    else:
        args.parameters = args.parameters.expanduser().resolve()
    if args.thrust_curve is None:
        args.thrust_curve = (
            args.repo / "source_data" / "R7_SIMLE" / "R7_OUTPUT" / "thrust_source.csv"
        )
    else:
...
def load_real_flight(args: argparse.Namespace) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    required = {*SENSOR_COLUMNS, Z_POSITION_COLUMN, Z_ACCELERATION_COLUMN}
