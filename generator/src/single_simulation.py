...
from rocketpy import Accelerometer, Barometer, Flight, Gyroscope
...
def run_single_simulation(
    ...
    for sensor_tuple in rocket.sensors:
        ...
        if isinstance(sensor, (Accelerometer, Gyroscope, ScalarSensor, Barometer)):
            accel_data.append({"df": frame, "name": sensor.name})
    ...
    if accel_data:
        ...
        all_accels_df["Best_Acc_X"] = get_best_acceleration(
            all_accels_df["Acceleration_X"], "X", all_accels_df, acceleration_thresholds
        )
        all_accels_df["Best_Acc_Y"] = get_best_acceleration(
            all_accels_df["Acceleration_Y"], "Y", all_accels_df, acceleration_thresholds
        )
        all_accels_df["Best_Acc_Z"] = get_best_acceleration(
            all_accels_df["Acceleration_Z"], "Z", all_accels_df, acceleration_thresholds
        )

        real_w1, real_w2, real_w3 = (
            fast_extract(current_flight.w1, times_array),
            fast_extract(current_flight.w2, times_array),
            fast_extract(current_flight.w3, times_array),
        )
        all_accels_df["Best_AngVel_X"] = get_best_angular_velocity(
            real_w1, "X", all_accels_df, angular_velocity_thresholds
        )
        all_accels_df["Best_AngVel_Y"] = get_best_angular_velocity(
            real_w2, "Y", all_accels_df, angular_velocity_thresholds
        )
        all_accels_df["Best_AngVel_Z"] = get_best_angular_velocity(
            real_w3, "Z", all_accels_df, angular_velocity_thresholds
        )
