...
def download_yearly_weather(year, longitude, latitude, path="../../source_data/ERA5_weather/"):
    ...
    if not os.path.exists(target_single):
        Log.print_info(
            f"Queueing API request for {year} SINGLE levels (This will take a minute or two...)"
        )
    ...
    if not os.path.exists(target_levels):
        Log.print_info(
            f"Queueing API request for {year} PRESSURE levels (This will take a minute or two...)"
        )
